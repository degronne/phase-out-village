...
export default function onboardingDialog({
  ...
  return (
    <Dialog open={open} onClose={finish}>
      <div className="ob-overlay" role="dialog" aria-modal="true">
        <div className="ob-wrap">
          <button className="ob-skip" onClick={finish} aria-label="Hopp over">
            Hopp over
          </button>
          <div
            ref={scrollerRef}
            onScroll={onScroll}
            className="ob-scroller"
            aria-roledescription="carousel"
            aria-label="introduksjon til spillet"
          >
            {slides.map((s, i) => (
              <section
                key={i}
                className="ob-slide"
                aria-label={`${s.title} (${i + 1} av ${slides.length})`}
              >
